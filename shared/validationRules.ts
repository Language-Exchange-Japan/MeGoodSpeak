...
export const ValidationHelpers = {
  ...
  validateUsername(username: string): { isValid: boolean; error?: string } {
    ...
    if (username.length < SHARED_VALIDATION_RULES.USERNAME.MIN_LENGTH) {
      return {
        isValid: false,
        error: `Username must be at least ${SHARED_VALIDATION_RULES.USERNAME.MIN_LENGTH} characters`,
      };
    }
    if (username.length > SHARED_VALIDATION_RULES.USERNAME.MAX_LENGTH) {
      return {
        isValid: false,
        error: `Username cannot exceed ${SHARED_VALIDATION_RULES.USERNAME.MAX_LENGTH} characters`,
      };
    }
  ...
  validatePassword(password: string): { isValid: boolean; error?: string } {
    ...
    if (password.length < SHARED_VALIDATION_RULES.PASSWORD.MIN_LENGTH) {
      return {
        isValid: false,
        error: `Password must be at least ${SHARED_VALIDATION_RULES.PASSWORD.MIN_LENGTH} characters`,
      };
    }
    if (password.length > SHARED_VALIDATION_RULES.PASSWORD.MAX_LENGTH) {
      return {
        isValid: false,
        error: `Password cannot exceed ${SHARED_VALIDATION_RULES.PASSWORD.MAX_LENGTH} characters`,
      };
    }
    return { isValid: true };
  },
  ...
  validateAge(age: number): { isValid: boolean; error?: string } {
    if (age < SHARED_VALIDATION_RULES.AGE.MIN) {
      return {
        isValid: false,
        error: `Age must be at least ${SHARED_VALIDATION_RULES.AGE.MIN}`,
      };
    }
    if (age > SHARED_VALIDATION_RULES.AGE.MAX) {
      return {
        isValid: false,
        error: `Age cannot exceed ${SHARED_VALIDATION_RULES.AGE.MAX}`,
      };
    }
    return { isValid: true };
  },
  ...
  validateBio(bio: string): { isValid: boolean; error?: string } {
    if (bio && bio.length > SHARED_VALIDATION_RULES.BIO.MAX_LENGTH) {
      return {
        isValid: false,
        error: `Bio cannot exceed ${SHARED_VALIDATION_RULES.BIO.MAX_LENGTH} characters`,
      };
    }
    return { isValid: true };
  },
